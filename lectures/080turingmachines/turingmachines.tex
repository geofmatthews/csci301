...
\begin{array}{cccccccc}
 ...
 &  \langle M_3 \rangle &  \langle M_4 \rangle &  \langle M_5 \rangle
  & ...\\
  M_0 & \mybf accept & accept & accept & reject & accept & reject & ...\\
  M_1 & accept & \mybf reject & accept & accept & accept & reject & ...\\
  M_2 & accept & reject & \mybf accept & accept & accept & reject & ...\\
  M_3 & accept & accept & reject & \mybf reject & accept & accept & ...\\
  M_4 & reject & accept & accept & reject & \mybf accept & accept & ...\\
  M_5 & reject & reject & accept & accept & accept & \mybf reject & ...\\
  ... &  ... &  ... &  ... &  ... &  ... &  ... &  ... 
\end{array}
\]}}
...
\item If $H$ says   $Q(\langle Q\rangle) $ terminates,
    $Q(\langle Q\rangle) $ does not terminate.
\item If $H$ says   $Q(\langle Q\rangle) $ does not terminate,
    $Q(\langle Q\rangle) $  terminates.
\item
  $Q(\langle Q\rangle) $
...
\item  $\{\langle M\rangle\mid \mbox{$M$ accepts only input of length $10$}\}$
\item $\{\langle  M\rangle\mid \mbox{$M$ accepts all inputs}\}$
\item $\{\langle  M\rangle\mid \mbox{$M$ rejects all inputs}\}$
\end{enumerate}
...
\begin{itemize}
\item
  Note that it is easy to find a machine in each of these sets.
\item
  It is also easy to find a machine not in each of these sets.
\item
  What {\em cannot} be done is find a machine that will
  decide if {\em another} machine is in one of these sets.
\end{itemize}
...
\item A {\em computable real number} is one for which there is a
  ...
  \begin{itemize}
\item $\pi$ and $\sqrt{2}$ are computable, even though their decimal
  expansions never end.
  \end{itemize}
